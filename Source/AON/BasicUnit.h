...
UCLASS()
class AON_API ABasicUnit : public ACharacter
{
...
public:	
	...
	//for UI
	UFUNCTION()
	void OnMouseClicked(class UPrimitiveComponent* ClickedComp, FKey ButtonPressed);
	...
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	bool TriggerSkill(int32 index, FVector Pos, class ABasicUnit* CurrentTarget);
	...
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	bool UseSkill(EHeroActionStatus SpellType, int32 index, FVector VFaceTo, FVector Pos, class ABasicUnit* victim);

	UFUNCTION(BlueprintCallable, Category = "MOBA")
	int32 GetCurrentSkillIndex();

	UFUNCTION(BlueprintCallable, Category = "MOBA")
	class AHeroSkill* GetCurrentSkill();
	...
	//加上buff
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	void AddBuff(AHeroBuff* buff, class ABasicUnit* caster);
	...
	//加入Buff時取代同名的Buff
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	void AddUniqueBuff(AHeroBuff* buff, class ABasicUnit* caster);
	
	//移除所有同名的Buff
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	void RemoveBuffByName(FString name, class ABasicUnit* caster);

	//移除Buff
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	void RemoveBuff(AHeroBuff* buff, class ABasicUnit* caster);
	
	//移除所有正面狀態
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	void RemoveFriendlyBuff(class ABasicUnit* caster);

	//移除所有負面狀態
	UFUNCTION(BlueprintCallable, Category = "MOBA")
	void RemoveUnfriendlyBuff(class ABasicUnit* caster);
	...
	bool Pickup(class AEquipment* equ);

	bool ThrowEquipment(class AEquipment* equ, FVector pos);

	bool HasEquipment(class AEquipment* equ);

	UFUNCTION(BlueprintImplementableEvent, Category = "MOBA")
	void OnAnimaStatusChanged(int32 _LastAnimaStatus, int32 _AnimaStatus);

	class UWebInterfaceJsonObject* BuildJsonObject();

	UFUNCTION(BlueprintCallable, Category = "MOBA")
	virtual class UWebInterfaceJsonValue* BuildJsonValue();
	...
	TMap<FString, class UParticleSystemComponent*> Particles;
	...
	class UArrowComponent * PositionOnHead;

	//手動設定的腳底位置
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "MOBA|Interaction")
	class UArrowComponent * PositionUnderFoot;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "MOBA|Interaction")
	class UAudioComponent * AttackStartSFX;
	
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "MOBA|Interaction")
	class UAudioComponent * AttackLandedSFX;
	...
	//移動攻擊時，遇到想攻擊的敵人
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "MOBA")
	class ABasicUnit* MovingAttackTarget = nullptr;
	...
	TArray<class AHeroSkill*> Skills;
	//隊伍id
	...
	class ASkillHintActor* CurrentSkillHint;
