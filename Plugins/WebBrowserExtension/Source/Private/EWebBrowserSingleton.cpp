...
FString FEWebBrowserSingleton::ApplicationCacheDir() const
{
#if PLATFORM_MAC
...
class FWebBrowserWindowFactory
	: public IEWebBrowserWindowFactory
{
public:
...
FEWebBrowserSingleton::FEWebBrowserSingleton(const FEWebBrowserInitSettings& WebBrowserInitSettings)
#if WITH_CEF3
...
{
#if WITH_CEF3
	// The FEWebBrowserSingleton must be initialized on the game thread
	...
	{
		UE_LOG(LogEWebBrowser, Error, TEXT("Chromium Resources information not found at: %s."), *ResourcesPath);
	}
	...
	{
		UE_LOG(LogEWebBrowser, Error, TEXT("Chromium Locales information not found at: %s."), *LocalesPath);
	}
	...
	{
		UE_LOG(LogEWebBrowser, Error, TEXT("UnrealCEFSubProcess.exe not found, check that this program has been built and is placed in: %s."), *SubProcessPath);
	}
...
#if WITH_CEF3
void FEWebBrowserSingleton::HandleRenderProcessCreated(CefRefPtr<CefListValue> ExtraInfo)
{
...
FEWebBrowserSingleton::~FEWebBrowserSingleton()
{
#if WITH_CEF3
...
TSharedRef<IEWebBrowserWindowFactory> FEWebBrowserSingleton::GetWebBrowserWindowFactory() const
{
...
TSharedPtr<IEWebBrowserWindow> FEWebBrowserSingleton::CreateBrowserWindow(const FECreateBrowserWindowSettings& WindowSettings)
{
#if WITH_CEF3
	...
	if (AllowCEF)
	{
		...
		{
			const FEBrowserContextSettings Context = WindowSettings.Context.GetValue();
...
bool FEWebBrowserSingleton::Tick(float DeltaTime)
{
#if WITH_CEF3
	FScopeLock Lock(&WindowInterfacesCS);
...
FString FEWebBrowserSingleton::GetCurrentLocaleCode()
{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void FEWebBrowserSingleton::DeleteBrowserCookies(FString URL, FString CookieName, TFunction<void(int)> Completed)
{
...
TSharedPtr<IEWebBrowserCookieManager> FEWebBrowserSingleton::GetCookieManager(TOptional<FString> ContextId) const
{
	if (!ContextId.IsSet())
	{
...
bool FEWebBrowserSingleton::RegisterContext(const FEBrowserContextSettings& Settings)
{
#if WITH_CEF3
...
bool FEWebBrowserSingleton::UnregisterContext(const FString& ContextId)
{
#if WITH_CEF3
...
bool FEWebBrowserSingleton::RegisterSchemeHandlerFactory(FString Scheme, FString Domain, IEWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
#if WITH_CEF3
...
bool FEWebBrowserSingleton::UnregisterSchemeHandlerFactory(IEWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
#if WITH_CEF3
