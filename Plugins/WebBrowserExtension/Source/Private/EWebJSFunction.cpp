...
FEWebJSParam::FEWebJSParam(const FEWebJSParam& Other)
	: Tag(Other.Tag)
{
	switch (Other.Tag)
	{
		...
		case PTYPE_STRUCT:
			...
			break;
		case PTYPE_ARRAY:
			ArrayValue = new TArray<FEWebJSParam>(*Other.ArrayValue);
			break;
		case PTYPE_MAP:
			MapValue = new TMap<FString, FEWebJSParam>(*Other.MapValue);
			break;
	}
}

void FEWebJSCallbackBase::Invoke(int32 ArgCount, FEWebJSParam Arguments[], bool bIsError) const
{
	TSharedPtr<FEWebJSScripting> Scripting = ScriptingPtr.Pin();
