...
bool FCEFWebBrowserWindowEx::OnJSDialog(CefJSDialogHandler::JSDialogType DialogType, const CefString& MessageText, const CefString& DefaultPromptText, CefRefPtr<CefJSDialogCallback> Callback, bool& OutSuppressMessage)
{
	...
	{
		TSharedPtr<IEWebBrowserDialog> Dialog(new FCEFWebBrowserDialog(DialogType, MessageText, DefaultPromptText, Callback));
		EEWebBrowserDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IEWebBrowserDialog>(Dialog));
		switch (EventResponse)
		{
		case EEWebBrowserDialogEventResponse::Handled:
			Retval = true;
			break;
		case EEWebBrowserDialogEventResponse::Continue:
			...
			break;
		case EEWebBrowserDialogEventResponse::Ignore:
			OutSuppressMessage = true;
			Retval = false;
			break;
		case EEWebBrowserDialogEventResponse::Unhandled:
		default:
			Retval = false;
			break;
		}
...
bool FCEFWebBrowserWindowEx::OnBeforeUnloadDialog(const CefString& MessageText, bool IsReload, CefRefPtr<CefJSDialogCallback> Callback)
{
	...
	if ( OnShowDialog().IsBound() )
	{
		TSharedPtr<IEWebBrowserDialog> Dialog(new FCEFWebBrowserDialog(MessageText, IsReload, Callback));
		EEWebBrowserDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IEWebBrowserDialog>(Dialog));
		switch (EventResponse)
		{
		case EEWebBrowserDialogEventResponse::Handled:
...
void FCEFWebBrowserWindowEx::NotifyDocumentLoadingStateChange(bool IsLoading)
{
	...
	if (IsLoading || DocumentState != EEWebBrowserDocumentState::Error)
	{
		ErrorCode = 0;
		DocumentState = IsLoading
			? EEWebBrowserDocumentState::Loading
			: EEWebBrowserDocumentState::Completed;
		DocumentStateChangedEvent.Broadcast(DocumentState);
	}
	...
	if (InternalCefBrowser != nullptr && InternalCefBrowser->IsSame(Browser))
	{
		...
		if (MainFrame.get() != nullptr)
		{
			if(OnBeforeBrowse().IsBound())
			{
				...
				FEWebNavigationRequest RequestDetails;
...
void FCEFWebBrowserWindowEx::OnBrowserClosed()
{
	...
	{
		OnCloseWindow().Execute(TWeakPtr<IEWebBrowserWindow>(SharedThis(this)));
	}
