...
public:
	...
	bool SetConverted(CefRefPtr<ContainerType> Container, KeyType Key, FEWebJSParam& Param)
	{
		switch (Param.Tag)
		{
			case FEWebJSParam::PTYPE_NULL:
				return Container->SetNull(Key);
			case FEWebJSParam::PTYPE_BOOL:
				return Container->SetBool(Key, Param.BoolValue);
			case FEWebJSParam::PTYPE_DOUBLE:
				return Container->SetDouble(Key, Param.DoubleValue);
			case FEWebJSParam::PTYPE_INT:
				return Container->SetInt(Key, Param.IntValue);
			case FEWebJSParam::PTYPE_STRING:
			{
				CefString ConvertedString = **Param.StringValue;
				return Container->SetString(Key, ConvertedString);
			}
			case FEWebJSParam::PTYPE_OBJECT:
			{
			...
			case FEWebJSParam::PTYPE_STRUCT:
			{
	...
	void InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FEWebJSParam Arguments[], bool bIsError=false) override;
