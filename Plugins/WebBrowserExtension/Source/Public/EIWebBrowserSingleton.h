...
struct WEBBROWSEREXTENSION_API FEBrowserContextSettings
{
	FEBrowserContextSettings(const FString& InId)
		: Id(InId)
		, AcceptLanguageList()
...
struct WEBBROWSEREXTENSION_API FECreateBrowserWindowSettings
{

	FECreateBrowserWindowSettings()
	...
	TOptional<FEBrowserContextSettings> Context;
};

/**
 * A singleton class that takes care of general web browser tasks
 */
class WEBBROWSEREXTENSION_API IEWebBrowserSingleton
{
public:
	/**
	 * Virtual Destructor
	 */
	virtual ~IEWebBrowserSingleton() {};

	/** @return A factory object that can be used to construct additional WebBrowserWindows on demand. */
	virtual TSharedRef<IEWebBrowserWindowFactory> GetWebBrowserWindowFactory() const = 0;
	...
	virtual TSharedPtr<IEWebBrowserWindow> CreateBrowserWindow(const FECreateBrowserWindowSettings& Settings) = 0;
	...
	virtual TSharedPtr<class IEWebBrowserCookieManager> GetCookieManager() const = 0;

	virtual TSharedPtr<class IEWebBrowserCookieManager> GetCookieManager(TOptional<FString> ContextId) const = 0;

	virtual bool RegisterContext(const FEBrowserContextSettings& Settings) = 0;
	 ...
	 */
	virtual bool RegisterSchemeHandlerFactory(FString Scheme, FString Domain, IEWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory) = 0;
	 ...
	 */
	virtual bool UnregisterSchemeHandlerFactory(IEWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory) = 0;
